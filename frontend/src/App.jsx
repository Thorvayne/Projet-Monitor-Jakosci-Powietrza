                ...
                <>
                  ...
                  <div className="max-h-40 overflow-y-auto mb-2">
                    {favorites.length === 0 ? <p className="p-2 text-sm text-gray-400 italic">Brak ulubionych</p> : 
                     favorites.map(fav => (
  <div key={fav} className="flex justify-between items-center">
    
    <button 
      onClick={() => { handleCityChange(fav); setMenuOpen(false); }} 
      className="block w-full text-left p-2 hover:bg-blue-50 hover:text-blue-600 font-medium rounded-lg transition"
    >
      {fav}
    </button>
                ...
                {data.available_stations && data.available_stations.length > 1 ? (
                  <div className="mb-5">
                    <select 
                      ...
                      className="text-[10px] p-2.5 rounded-xl border bg-gray-50 dark:bg-gray-700 dark:border-gray-600 dark:text-white focus:ring-2 focus:ring-blue-400 outline-none w-full max-w-[300px] mx-auto block text-center uppercase tracking-widest cursor-pointer hover:bg-gray-100 transition font-bold"
                    >
